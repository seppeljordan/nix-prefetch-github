...
class Issue22Tests(TestCase):
    @property
    def nixos_secure_factory_ls_remote_output(self):
        with open(
            os.path.join(
    ...
    def test_prefetch_sensu_go_5_11(self) -> None:
        ...
        sequence = [
            (
                ...
                lambda _: self.nixos_secure_factory_ls_remote_output,
            ),
        ]
        effect = prefetch_github(
            self.url_hasher, repository=repository, prefetch=False, rev="jrg/mvp"
        )
